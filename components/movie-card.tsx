    ...
    <Card className="overflow-hidden border-border bg-card animate-in fade-in slide-in-from-bottom-4 duration-500">
      <CardContent className="p-0">
        <div className="flex flex-col sm:flex-row">
          {/* Poster */}
          <div className="relative shrink-0 sm:w-64 aspect-[2/3] sm:aspect-auto sm:h-auto bg-secondary">
            {hasPoster ? (
              <Image
                src={movie.Poster}
          ...
          <div className="flex flex-1 flex-col gap-4 p-5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <MetaItem icon={<Calendar className="h-3.5 w-3.5" />} label="Director" value={movie.Director} />
              <MetaItem icon={<Globe className="h-3.5 w-3.5" />} label="Cast" value={movie.Actors} />
              {movie.Awards && movie.Awards !== "N/A" && (
                <MetaItem icon={<Award className="h-3.5 w-3.5" />} label="Awards" value={movie.Awards} />
              )}
              {movie.BoxOffice && movie.BoxOffice !== "N/A" && (
                <MetaItem icon={<DollarSign className="h-3.5 w-3.5" />} label="Box Office" value={movie.BoxOffice} />
              )}
              <MetaItem icon={<Clock className="h-3.5 w-3.5" />} label="Released" value={movie.Released} />
              <MetaItem icon={<Globe className="h-3.5 w-3.5" />} label="Language" value={movie.Language} />
            </div>
          </div>
        </div>
      </CardContent>
